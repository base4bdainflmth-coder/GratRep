...
import {
  ...
  updateConfig, updateUsersConfig, getNextMapNumber
...
const extractYear = (mapId: string) => {
  const parts = mapId.split('/');
  if (parts.length > 1) {
    const yearPart = parts[1].split('-')[0].trim();
    return yearPart.substring(0, 4);
  }
  return '';
};

// --- Funções Auxiliares de Datas Inteligentes e SLA (Globais) ---
export const parseBRDateGlobal = (dateStr: string) => {
...
export const calculateDaysGlobal = (
  label: string,
  dataRemessaStr: string,
  dataSaidaStr: string,
  dataCmLStr: string,
  dataPgStr: string,
  ...
  const dRemessa = parseBRDateGlobal(dataRemessaStr);
  const dSaida = parseBRDateGlobal(dataSaidaStr);
  const dCml = parseBRDateGlobal(dataCmLStr);
  const dPg = parseBRDateGlobal(dataPgStr);
  const dEvento = parseBRDateGlobal(dataEventoStr);

  const diff = (d1: Date, d2: Date) => Math.floor((d1.getTime() - d2.getTime()) / (1000 * 60 * 60 * 24));

  if (label.includes('Evento -> Remessa')) {
    if (!dRemessa || !dEvento) return '-';
    return diff(dRemessa, dEvento);
  }
  if (label.includes('Recb -> Remessa DE')) {
    if (!dRemessa) return '-';
    return dSaida ? diff(dSaida, dRemessa) : diff(today, dRemessa);
  }
  if (label.includes('Remessa DE -> CML')) {
    if (!dSaida) return '-';
    // Se CML="", mede atraso ate hoje; senão mede envio. Porém para o Esc Sp, se devolveu ou cancelou antes, não continua. Mantemos generico:
    return dCml ? diff(dCml, dSaida) : diff(today, dSaida);
  }
  if (label.includes('OM -> Autz')) {
    if (!dRemessa) return '-';
    return dPg ? diff(dPg, dRemessa) : diff(today, dRemessa);
  }
  if (label.includes('Bda -> DE -> Autz')) {
    if (!dPg || !dSaida) return '-';
    return diff(dPg, dSaida);
  }
  if (label.includes('DE -> CML -> Autz')) {
    if (!dPg || !dCml) return '-';
    return diff(dPg, dCml);
  }

  return '-';
};

export const getStyleForDaysGlobal = (label: string, value: number | string) => {
  if (value === '-') return 'text-gray-800';
  const num = typeof value === 'string' ? parseInt(value) : value;

  if (label.includes('Evento -> Remessa')) {
    if (num >= 60) return 'text-white bg-black px-2 py-0.5 rounded font-black border border-white/20 shadow-sm';
    if (num >= 50) return 'text-white bg-red-600 px-2 py-0.5 rounded font-bold border border-red-700 shadow-sm';
  }
  if (label.includes('Recb -> Remessa DE') || label.includes('Remessa DE -> CML')) {
    if (num >= 10) return 'text-white bg-black px-2 py-0.5 rounded font-black border border-white/20 shadow-sm';
    if (num >= 5) return 'text-white bg-red-600 px-2 py-0.5 rounded font-bold border border-red-700 shadow-sm';
  }
  return 'text-army-800 bg-army-100 px-2 py-0.5 rounded font-bold border border-army-200 shadow-sm';
};
          ...
          {activeTab === 'auxiliar' ? (
            <div className="space-y-6">
              <div className="bg-sky-50 p-4 rounded-xl border border-sky-100">
                <label className="block text-xs font-bold text-sky-700 uppercase mb-2">Exercício Corrente</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <ListEditor title="Eventos" items={eventos} setItems={setEventos} />
                <ListEditor title="Motivos Devolução" items={motivos} setItems={setMotivos} />
                <ListEditor title="Destinos" items={destinos} setItems={setDestinos} />
              </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Email Administrador</label>
                  <input type="email" className="w-full p-2 border rounded text-sm" value={adminEmail} onChange={e => setAdminEmail(e.target.value)} />
                </div>
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Senha Administrador</label>
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="text-gray-400 uppercase text-[10px] border-b">
                        <th className="p-2 text-left">OM</th>
...
const DetailsModal: React.FC<{ data: MapData; onClose: () => void }> = ({ data, onClose }) => {
  ...
  const calculateDays = (label: string): number | string => {
    return calculateDaysGlobal(
      label,
      getVal(['Data DIEx Remessa']),
      getVal(['Data DIEx Saída']),
      getVal(['Data DIEx da 1ª DE ao CML']),
      getVal(['Data Doc Autz Pg']),
      getVal(['Ult Dia', 'Ultimo Dia'])
    );
  };

  const getStyleForDays = (label: string, value: number | string) => {
    return getStyleForDaysGlobal(label, value);
  };

  const Field = ({ label, patterns, full = false, isDays = false }: { label: string, patterns: string[], full?: boolean, isDays?: boolean }) => {
    const valStr = isDays ? calculateDays(label) : getVal(patterns);
    const styleClass = isDays ? getStyleForDays(label, valStr) : 'text-gray-800';

    return (
      <div className={`flex flex-col items-start ${full ? 'col-span-full' : ''}`}>
        <span className="text-[10px] uppercase font-bold text-gray-400 tracking-wider mb-0.5">{label}</span>
        <span className={`text-sm font-semibold break-words inline-block ${styleClass}`}>{valStr}</span>
      </div>
    );
  };
        ...
        <div className="overflow-y-auto p-6 space-y-6 bg-white">
          ...
          <section className="bg-emerald-50 rounded-xl p-4 border border-emerald-200 ring-1 ring-emerald-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              <Field label="Dias (OM -> Autz)" patterns={['Dias Recb OM -> Autz Pg']} isDays={true} />
              <Field label="Dias (Bda -> DE -> Autz)" patterns={['Dias Enc Bda -> DE -> Autz Pg']} isDays={true} />
              <Field label="Dias (DE -> CML -> Autz)" patterns={['Dias Enc DE -> CML -> Autz Pg']} isDays={true} />
            </div>
          </section>
...
const SmartEditInput = ({ label, exactHeader, type = 'text', numericOnly = false, formData, cleanHeaders, onChange }: { label: string, exactHeader: string, type?: string, numericOnly?: boolean, formData: Record<string, string>, cleanHeaders: string[], onChange: (k: string, v: string) => void }) => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (numericOnly) newValue = newValue.replace(/\D/g, '');
  ...
  return (
    <div className="flex flex-col">
      ...
      <input type={type} disabled={isCalculated} placeholder={numericOnly ? "somente números" : ""} className={`p-2 border rounded text-sm outline-none w-full ${isCalculated ? 'bg-gray-100 text-gray-500 border-gray-200 cursor-not-allowed' : 'bg-white border-gray-300 focus:ring-2 focus:ring-army-500'}`} value={val} onChange={handleInputChange} />
    </div>
  );
};
...
const EditMapModal: React.FC<{ data: MapData; auxData: AuxiliarData | null; onClose: () => void; onSuccess: (msg: string) => void }> = ({ data, auxData, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSubmitting(true);
    const calcStatus = () => {
      if (formData['Doc Autorização de Pagamento'] || formData['Data Doc Autz Pg']) return 'Pagamento autorizado.';
      if (formData['Nr DIEx Devol'] || formData['Data DIEx Devol']) {
        if (formData['Motivo'] && formData['Motivo'].includes('Processo não analisado pelo CML')) return 'Cancelado (DEA)';
        return 'Processo devolvido';
      }
      if (formData['DIEx da 1ª DE ao CML'] || formData['Data DIEx da 1ª DE ao CML']) return 'Aguardando autorização do CML';
      if (formData['Nr DIEx Saída'] || formData['Data DIEx Saída']) return 'Processo encaminhado ao Esc Sp';
      if (formData['Nr DIEx Remessa 4 Bda'] || formData['Data DIEx Remessa 4 Bda']) return 'Encaminhado à 4ª Bda.';
      return 'Não encaminhado à Bda';
    };

    const newStatus = calcStatus();
    formData['Situação'] = newStatus;
        ...
        <form id="edit-form" onSubmit={handleSubmit} className="overflow-y-auto p-6 space-y-6">
          <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <SmartEditInput label="Nr DIEx" exactHeader="Nr DIEx Remessa 4 Bda" formData={formData} numericOnly={true} cleanHeaders={data.cleanHeaders} onChange={handleChange} />
          ...
          <div className="bg-sky-50 p-4 rounded-lg border border-sky-100">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <SmartEditInput label="Nr DIEx Saída" exactHeader="Nr DIEx Saída" formData={formData} numericOnly={true} cleanHeaders={data.cleanHeaders} onChange={handleChange} />
          ...
          <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-100">
            ...
            <div className="grid grid-cols-2 gap-4">
              <SmartEditInput label="DIEx DE ao CML" exactHeader="DIEx da 1ª DE ao CML" formData={formData} numericOnly={true} cleanHeaders={data.cleanHeaders} onChange={handleChange} />
          ...
          <div className="bg-red-50 p-4 rounded-lg border border-red-200">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <SmartEditInput label="Nr DIEx Devol" exactHeader="Nr DIEx Devol" formData={formData} numericOnly={true} cleanHeaders={data.cleanHeaders} onChange={handleChange} />
...
const NewMapForm: React.FC<{ user: User; onClose: () => void; onSuccess: (msg: string) => void }> = ({ user, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    const year = formData.ultDiaEvento ? formData.ultDiaEvento.split('-')[0] : new Date().getFullYear().toString();
    getNextMapNumber(year).then(setNextNumber);
  }, [formData.ultDiaEvento]);
  ...
  const executeSubmission = async () => {
    ...
    const payload = {
      ...formData,
      ultDiaEvento: formData.ultDiaEvento,
      dataDiex: formData.dataDiex,
      nrDiex: formData.nrDiex,
    ...
    if (success) {
      ...
      onSuccess("Erro ao criar mapa. Tente de novo.");
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.evento || !formData.valor || !formData.docAutoriza || !formData.selectedOM) {
      onSuccess("Por favor, preencha todos os campos obrigatórios.");
      return;
    }

    if (isLate(formData.ultDiaEvento) && !formData.nrDiex) {
  ...
  const getPreviewMapa = () => {
    const year = formData.ultDiaEvento ? formData.ultDiaEvento.split('-')[0] : new Date().getFullYear().toString();
    return `${nextNumber || '...'}/${year} - 4 Bda/${formData.selectedOM || '...'}`;
  };
            ...
            {user.role === UserRole.ADMIN && (
              <div className="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <select required className="w-full p-2.5 border border-gray-300 rounded-lg text-sm bg-white" value={formData.selectedOM} onChange={e => setFormData({ ...formData, selectedOM: e.target.value })}>
                  ...
                  {usersInfo.users.map(u => <option key={u.om} value={u.om}>{u.om}</option>)}
                </select>
              </div>
            )}

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2 bg-gray-50 border border-gray-200 p-4 rounded-xl">
                <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Próximo Mapa Disponível (Automático)</label>
                <div className="flex items-center space-x-2">
                  <span className="text-xl font-mono font-bold text-army-700">{getPreviewMapa()}</span>
                  {nextNumber === null && <Loader2 className="w-4 h-4 animate-spin text-army-600" />}
                </div>
                <p className="text-[10px] text-gray-400 mt-2 italic">O número do mapa é sequencial global e será confirmado ao salvar.</p>
              </div>
              <div>
              ...
              <div>
                <label className={`flex items-center text-xs font-bold uppercase mb-1 ${user.role === UserRole.ADMIN ? 'text-gray-500' : 'text-gray-400'}`}>
                  Nr DIEx Remessa
                </label>
                <input
                  disabled={user.role !== UserRole.ADMIN}
                  type="text"
                  placeholder={user.role === UserRole.ADMIN ? "Digite o número" : "Somente números"}
                  className={`w-full p-2 border rounded-lg text-sm ${user.role === UserRole.ADMIN ? 'border-gray-300 bg-white' : 'border-gray-200 bg-gray-50 text-gray-400 cursor-not-allowed'}`}
                  value={formData.nrDiex}
                  onChange={e => setFormData({ ...formData, nrDiex: e.target.value.replace(/\D/g, '') })}
                />
              </div>
              <div>
                <label className={`block text-xs font-bold uppercase mb-1 ${user.role === UserRole.ADMIN ? 'text-gray-500' : 'text-gray-400'}`}>
                  Data DIEx
                </label>
                <input
                  disabled={user.role !== UserRole.ADMIN}
                  type="date"
                  className={`w-full p-2 border rounded-lg text-sm ${user.role === UserRole.ADMIN ? 'border-gray-300 bg-white' : 'border-gray-200 bg-gray-50 text-gray-400 cursor-not-allowed'}`}
                  value={formData.dataDiex}
                  onChange={e => setFormData({ ...formData, dataDiex: e.target.value })}
                />
              </div>
...
const SendMapModal: React.FC<{ data: MapData; user: User; onClose: () => void; onSuccess: (msg: string) => void }> = ({ data, user, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const headerSit = getHeader('situação') || getHeader('situacao');
    if (headerSit) {
      updates[headerSit.replace(/[\r\n]+/g, '').trim()] = 'Encaminhado para a 4ª Bda Inf L Mth.';
    }
...
const Dashboard: React.FC<DashboardProps> = ({ user, onLogout }) => {
  ...
  const stats = useMemo(() => {
    const todosMapasVisiveis = filteredData.length;

    const aprovados = filteredData.filter(i => i.situacao.trim() === 'Pagamento autorizado.').length;
    const pendentes = filteredData.filter(i => {
      const s = i.situacao.trim();
      return s.includes('Encaminhado para a 4ª Bda') || s.includes('Encaminhado à 4ª Bda') || s === 'Processo encaminhado ao Esc Sp' || s === 'Aguardando autorização do CML';
    }).length;

    const totalBase = aprovados + pendentes; // O novo TOTAL para as % dos primeiros

    const devolvidosCancelados = filteredData.filter(i => {
      const s = i.situacao.toLowerCase();
      return s.includes('devolvido') || s.includes('cancelado (dea)');
    }).length;

    const pctAprovados = totalBase > 0 ? ((aprovados / totalBase) * 100).toFixed(1) + '%' : '0%';
    const pctPendentes = totalBase > 0 ? ((pendentes / totalBase) * 100).toFixed(1) + '%' : '0%';
    const pctDevCanc = todosMapasVisiveis > 0 ? ((devolvidosCancelados / todosMapasVisiveis) * 100).toFixed(1) + '%' : '0%';
    ...
    <div className="min-h-screen bg-gray-100 font-sans pb-10">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <StatCard title="TOTAL (PG + PEND)" value={stats.totalBase} icon={FileSpreadsheet} color="text-blue-600 bg-blue-600" />
          <StatCard title="PG AUTORIZADO" value={stats.aprovados} subtitle={stats.pctAprovados} icon={CheckCircle2} color="text-emerald-600 bg-emerald-600" />
          <StatCard title="PENDENTES" value={stats.pendentes} subtitle={stats.pctPendentes} icon={RefreshCcw} color="text-orange-500 bg-orange-500" />
          <StatCard title="DEVOLVIDOS/CANCELADOS" value={stats.devolvidosCancelados} subtitle={`${stats.pctDevCanc} do GERAL`} icon={AlertCircle} color="text-[#ba3838] bg-[#ba3838]" />
        </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-100 border-b border-gray-200 text-[10px] uppercase font-bold text-gray-500">
                  ...
                  <th className="p-4 hidden md:table-cell">DIAS CORRIDOS</th>
              ...
              <tbody className="text-xs">
                {filteredData.map((row, idx) => {
                  ...
                  // Resolve SLA Chips para esta linha
                  const extractRowVal = (patterns: string[]) => {
                    const idx = row.cleanHeaders?.findIndex(h => h && patterns.some(p => h.toLowerCase().includes(p.toLowerCase())));
                    return (idx !== undefined && idx >= 0 && row.rawData) ? (row.rawData[idx] || '-') : '-';
                  };

                  const getSlaChips = () => {
                    const d1 = calculateDaysGlobal('Evento -> Remessa', extractRowVal(['Data DIEx Remessa']), extractRowVal(['Data DIEx Saída']), extractRowVal(['Data DIEx da 1ª DE ao CML']), extractRowVal(['Data Doc Autz Pg']), extractRowVal(['Ult Dia', 'Ultimo Dia']));
                    const d2 = calculateDaysGlobal('Recb -> Remessa DE', extractRowVal(['Data DIEx Remessa']), extractRowVal(['Data DIEx Saída']), extractRowVal(['Data DIEx da 1ª DE ao CML']), extractRowVal(['Data Doc Autz Pg']), extractRowVal(['Ult Dia', 'Ultimo Dia']));
                    const d3 = calculateDaysGlobal('Remessa DE -> CML', extractRowVal(['Data DIEx Remessa']), extractRowVal(['Data DIEx Saída']), extractRowVal(['Data DIEx da 1ª DE ao CML']), extractRowVal(['Data Doc Autz Pg']), extractRowVal(['Ult Dia', 'Ultimo Dia']));
                    const dCml = calculateDaysGlobal('OM -> Autz', extractRowVal(['Data DIEx Remessa']), extractRowVal(['Data DIEx Saída']), extractRowVal(['Data DIEx da 1ª DE ao CML']), extractRowVal(['Data Doc Autz Pg']), extractRowVal(['Ult Dia', 'Ultimo Dia']));

                    const chips = [];
                    if (d1 !== '-') chips.push({ label: 'OM', val: d1, style: getStyleForDaysGlobal('Evento -> Remessa', d1) });
                    if (d2 !== '-') chips.push({ label: 'BDA', val: d2, style: getStyleForDaysGlobal('Recb -> Remessa DE', d2) });
                    if (d3 !== '-') chips.push({ label: 'DE', val: d3, style: getStyleForDaysGlobal('Remessa DE -> CML', d3) });

                    // Chip CML logic: se dCml (Dias OM -> Autz que usa Data Autz e Data Remessa) não for vazio, adicionar
                    // A PARTIR DO MOMENTO EM QUE A SITUAÇÃO MUDA PARA Aguardando autorização do CML
                    const isCmlPhase = extractRowVal(['Data DIEx da 1ª DE ao CML']) !== '-' || row.situacao.includes('Aguardando autorização do CML') || row.situacao.includes('Pagamento');
                    if (dCml !== '-' && isCmlPhase) {
                      // Podemos usar a estilização básica de bg-army-100 para ele para se misturar, ou criar um estilo
                      const numCml = Number(dCml);
                      let styleCml = 'text-army-800 bg-army-100 px-2 py-0.5 rounded font-bold border border-army-200 shadow-sm';
                      // if you want some SLA colors for CML, apply them here. Currently using default.
                      chips.push({ label: 'CML', val: dCml, style: styleCml });
                    }

                    return chips;
                  };

                  const slaChips = getSlaChips();
                    ...
                    >
                      ...
                      <td className="p-4 hidden md:table-cell">
                        <div className="flex space-x-1 flex-wrap">
                          {slaChips.length > 0 ? slaChips.map((c, i) => (
                            <span key={i} className={`text-[9px] px-1.5 py-0.5 rounded shadow-sm whitespace-nowrap mb-1 ${c.style}`} title={`Atraso no trâmite da ${c.label}`}>
                              {c.label}: {c.val}d
                            </span>
